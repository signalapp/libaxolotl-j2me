...
public class SimultaneousInitiateTest extends TestCase {
  ...
  private static final ECKeyPair aliceSignedPreKey = Curve.generateKeyPair(new FakeSecureRandomProvider());
  private static final ECKeyPair bobSignedPreKey   = Curve.generateKeyPair(new FakeSecureRandomProvider());
      ...
      InvalidKeyIdException, NoSessionException
  {
    ...
    SessionBuilder aliceSessionBuilder = new SessionBuilder(new FakeSecureRandomProvider(), aliceStore, BOB_RECIPENT_ID, 1);
    SessionBuilder bobSessionBuilder   = new SessionBuilder(new FakeSecureRandomProvider(), bobStore, ALICE_RECIPIENT_ID, 1);

    SessionCipher aliceSessionCipher = new SessionCipher(new FakeSecureRandomProvider(), aliceStore, BOB_RECIPENT_ID, 1);
    SessionCipher bobSessionCipher = new SessionCipher(new FakeSecureRandomProvider(), bobStore, ALICE_RECIPIENT_ID, 1);
...
//    CiphertextMessage lostMessageForAlice = bobSessionCipher.encrypt("sample message".getBytes());
  ...
  private PreKeyBundle createAlicePreKeyBundle(AxolotlStore aliceStore) throws InvalidKeyException {
    ECKeyPair aliceUnsignedPreKey   = Curve.generateKeyPair(new FakeSecureRandomProvider());
    int       aliceUnsignedPreKeyId = new Random().nextInt(Medium.MAX_VALUE);
    byte[]    aliceSignature        = Curve.calculateSignature(new FakeSecureRandomProvider(),
  ...
  private PreKeyBundle createBobPreKeyBundle(AxolotlStore bobStore) throws InvalidKeyException {
    ECKeyPair bobUnsignedPreKey   = Curve.generateKeyPair(new FakeSecureRandomProvider());
    int       bobUnsignedPreKeyId = new Random().nextInt(Medium.MAX_VALUE);
    byte[]    bobSignature        = Curve.calculateSignature(new FakeSecureRandomProvider(),
